...
// TestRetry_RetriesServerErrors verifies a 5xx response is retried up to
// the configured attempt count, per the cli.yml server.retry setting.
func TestRetry_RetriesServerErrors(t *testing.T) {
	...
	c := NewWithOptions(srv.URL, "", DefaultTimeout, 3, 0)
	body, err := c.Get("/api/v1/things", nil)
	require.NoError(t, err)
	assert.Equal(t, 3, calls)
	assert.JSONEq(t, `{"ok":true}`, string(body))
}

// TestRetry_DoesNotRetryClientErrors verifies a 4xx response other than
// 429 is returned on the first attempt, per PART 9's "4xx errors are
// never retried" rule.
...
// TestRetry_RetriesRateLimited verifies 429 is treated as retryable.
func TestRetry_RetriesRateLimited(t *testing.T) {
	...
	c := NewWithOptions(srv.URL, "", DefaultTimeout, 2, 0)
	_, err := c.Get("/api/v1/things", nil)
	require.Error(t, err)

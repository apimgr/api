...
// Sentinel errors returned by IPLookup. Callers use errors.Is to tell
// caller-input failures (safe to echo back) apart from internal failures,
// whose detail must stay server-side.
var (
	// ErrInvalidIP means the supplied value is not a parseable IP address.
	ErrInvalidIP = errors.New("invalid IP address")
	// ErrIPNotPermitted means the address is private, loopback or link-local.
	ErrIPNotPermitted = errors.New("lookup of private/loopback/link-local addresses is not permitted")
	// ErrIPLookupFailed wraps an internal GeoIP database failure.
	ErrIPLookupFailed = errors.New("IP lookup failed")
)
...
// cached sapics/ip-location-db database (no per-request outbound call).
...
func (s *Service) IPLookup(ipStr string) (*IPInfo, error) {
	...
	if isBlockedIP(ip) {
		return nil, ErrIPNotPermitted
	}

	entry, err := geoip.Get().Lookup(ip.String())
	if err != nil {
		return nil, fmt.Errorf("%w: %w", ErrIPLookupFailed, err)
	}

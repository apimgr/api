...
func New(cfg *config.Config) *http.Server {
	...
	// HTTP metrics are recorded for every request, including ones later
	// rejected by rate limiting or security headers, so the counters
	// reflect real traffic rather than only the accepted subset.
	r.Use(metrics.Get().Middleware)
	// The same traffic feeds the public healthz aggregates, which are counted
	// alongside the Prometheus metrics so both see identical request volume.
	r.Use(healthStatsMiddleware)
	// Language resolution runs after logging and before any handler so every
	// handler and template can read the negotiated language from the request
	// context (?lang= -> cookie -> Accept-Language -> "en").
	...
	r.Use(formInputMiddleware)
	r.Use(RateLimitMiddleware(cfg))
	r.Use(geoIPMiddleware(cfg))
	r.Use(corsMiddleware(cfg))
	r.Use(versionPurgeMiddleware)

	// Static files
	r.Handle("/static/*", staticAssetHandler())
	...
	// Guest preferences (PART 16). Theme and language live in cookies only;
	// export/import carry the same two values to another device without any
	// server-side record.
	r.Get("/server/preferences", preferencesPageHandler(cfg))
	r.Post("/server/preferences", preferencesSaveHandler)
	r.Get("/server/preferences/export", preferencesExportPageHandler(cfg))
	r.Get("/server/preferences/import", preferencesImportHandler)
	r.Get("/api/v1/server/preferences", apiPreferencesHandler)
	r.Post("/api/v1/server/preferences", apiPreferencesSaveHandler)
	r.Get("/api/v1/server/preferences/export", apiPreferencesExportHandler)
	r.Get("/api/v1/server/preferences/import", preferencesImportHandler)
	...
	// Metrics endpoints (PART 20 — INTERNAL only). Paths are fixed, not
	// operator-configurable: /server/metrics[/{service}] plus the versioned
	// API path, the unversioned API alias, and the root alias Prometheus
	// scrapers expect. Every alias mounts the SAME handler — never a
	// redirect. Each service carries its own mandatory bearer token.
	if cfg.Server.Metrics.Enabled {
		mountMetrics(r, cfg)
	}

	// Tor CLI-to-running-server control channel (AI.md PART 31.1) —
	// INTERNAL, loopback-source only, never in OpenAPI/well-known/
	// FeaturesInfo, never under /api/{api_version}. This is how the
	// separately-invoked `{project_name} tor ...` subcommand reaches the
	// Tor process the running server already owns.
	r.Route("/server/tor", func(r chi.Router) {
		r.Use(loopbackOnlyMiddleware)
		r.Get("/status", torStatusHandler)
		r.Post("/validate", torValidateHandler)
		r.Post("/restart", torRestartHandler)
		r.Post("/regenerate", torRegenerateHandler)
		r.Post("/vanity/start", torVanityStartHandler)
		r.Post("/vanity/apply", torVanityApplyHandler)
		r.Post("/import-keys", torImportKeysHandler)
	})
	...
	r.Get("/favicon.ico", faviconHandler(cfg))
	r.Get(brandingFaviconRoute, brandingImageHandler(cfg, func(c *config.Config) string { return c.Server.Branding.Favicon }))
	r.Get(brandingLogoRoute, brandingImageHandler(cfg, func(c *config.Config) string { return c.Server.Branding.Logo }))
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Route("/network", func(r chi.Router) {
			...
			r.Get("/ip/{ip}", apiNetworkIPLookupHandler)
	...
	// Give every parameterised route a parameterless variant so a plain HTML
	// form can drive the same handler with zero JavaScript.
	registerFormFallbacks(r)
...
type PageData struct {
	...
	PrefExportURL      string
	PrefExportCode     string
	Layout             string
	ActivePage         string
	PageTitle          string
	...
	TorRunning         bool
	OnionAddress       string
	// I2P fields mirror the Tor trio for the footer's second overlay row
	// (PART 16 fixed row order: Tor -> I2P -> page links). No eepsite
	// subsystem is wired yet, so they stay false/empty and the guarded row
	// drops out entirely, exactly as it does for an unpublished onion.
	I2PEnabled bool
	I2PRunning bool
	I2PAddress string
	// CSRFToken carries the per-request double-submit token so every
	// server-rendered mutating form can emit the hidden input PART 16 requires
	// without any JavaScript.
	CSRFToken string
	// FaviconURL and LogoURL are resolved from server.branding.favicon /
	// server.branding.logo (PART 16 "Image Sources"): the embedded default when
	// unset, the remote URL when one is configured, or the local serving route
	// when the operator pointed at a file on disk. LogoURL is empty when no
	// logo is configured, so the header falls back to the site title text.
	FaviconURL string
	LogoURL    string
}
...
func newPageData(cfg *config.Config, r *http.Request, activePage string) PageData {
	...
	data := PageData{
		// Title and description are operator branding (PART 16): the head
		// partial reads them from here so no site identity is hardcoded in a
		// template.
		SiteTitle:       cfg.Server.Branding.Title,
		SiteDescription: cfg.Server.Branding.Description,
		SiteIcon:        "🛠️",
		BaseURL:         baseURL,
		Theme:           string(theme),
		ThemeClass:      ThemeClass(theme),
		Lang:            lang,
		LangDir:         i18n.Dir(lang),
		Languages:       i18n.AvailableLanguages(),
		Layout:          "public",
		ActivePage:      activePage,
		Version:         Version,
		CommitID:        CommitID,
		BuildDate:       footerBuildDate(),
		CSRFToken:       CSRFTokenFromContext(r.Context()),
		FaviconURL:      brandingAssetURL(cfg.Server.Branding.Favicon, brandingFaviconRoute, brandingDefaultFavicon),
		LogoURL:         brandingAssetURL(cfg.Server.Branding.Logo, brandingLogoRoute, brandingDefaultLogo),
	}
	// The footer's overlay row is published only when Tor is actually
	// running with an address, per PART 16's
	// {{ if and .TorEnabled .TorRunning .OnionAddress }} guard.
	if m := tor.Get(); m != nil {
		data.TorEnabled = true
		data.TorRunning = m.Running()
		data.OnionAddress = m.OnionAddress()
	}
	return data
}

// footerBuildDate renders the compiled-in build timestamp in the footer
// format PART 16 mandates for {build_datetime}: "%B %d, %Y at %H:%M:%S %Z",
// which is zero-padded on the day. An unparseable or unset BuildDate yields
// an empty string so the footer's build row drops out entirely rather than
// showing a broken value.
func footerBuildDate() string {
	if BuildDate == "" {
		return ""
	}
	for _, layout := range []string{time.RFC3339, "2006-01-02T15:04:05Z", "2006-01-02 15:04:05"} {
		if t, err := time.Parse(layout, BuildDate); err == nil {
			return t.Format("January 02, 2006 at 15:04:05 MST")
		}
	}
	return ""
}
...
// templateFuncs exposes the i18n layer to templates so no user-facing string
// has to be hardcoded in a .tmpl file (AI.md PART 30). "t" translates a key,
// "tf" translates with {placeholder} substitution, "tp" selects a CLDR plural
// form, and "dir" yields the text direction for the <html dir> attribute.
func templateFuncs() template.FuncMap {
	return template.FuncMap{
		"t": func(lang, key string) string {
			return i18n.Translate(lang, key)
		},
		"tf": func(lang, key string, args map[string]string) string {
			return i18n.TranslateFormat(lang, key, args)
		},
		"tp": func(lang, group string, count int, args map[string]string) string {
			return i18n.TranslatePlural(lang, group, count, args)
		},
		"dir": i18n.Dir,
		// "asset" stamps a static asset path with the running build stamp so
		// no template emits a bare, indefinitely cacheable /static/... URL
		// (AI.md PART 9 "Asset Version-Busting").
		"asset": assetURL,
		// "csrfField" renders the hidden double-submit input every
		// server-rendered mutating form needs (AI.md PART 16).
		"csrfField": csrfFieldHTML,
		// "nextTheme" gives the theme toggle a server-computed target derived
		// from the theme in effect for this request (AI.md PART 16 "Theme
		// Cycle Logic"), so repeated clicks keep cycling.
		"nextTheme": NextTheme,
	}
}
...
func initTemplates() error {
	...
	publicPages := []string{
		// "healthz" is absent deliberately: the health page is rendered
		// directly by src/server/handler/health.go so the HTML, JSON and
		// plain-text variants all come from one HealthResponse.
		"index", "text", "crypto", "datetime", "network", "openapi", "error", "about", "privacy", "contact", "help", "preferences", "preferences_export",
	...
	for _, page := range publicPages {
		tmpl, err := template.New(page).Funcs(templateFuncs()).ParseFS(templatesFS,
	...
	for _, tp := range toolPages() {
		...
		tmpl, err := template.New(key).Funcs(templateFuncs()).ParseFS(templatesFS,
			"template/layout/public.tmpl",
			"template/partial/*.tmpl",
			"template/partial/public/*.tmpl",
			contentTemplate,
...
// mountMetrics registers the PART 20 metrics surface. The bare
// /server/metrics path is an alias for /server/metrics/prometheus (same
// handler, never a redirect), and the whole prefix is repeated at the
// versioned API path, the unversioned API alias, and — gated on
// server.metrics.root.enabled — the root alias Prometheus scrapers default
// to. Every service is wrapped in metrics.AuthMiddleware, which enforces
// the per-service bearer token in constant time.
func mountMetrics(r chi.Router, cfg *config.Config) {
	mc := cfg.Server.Metrics
	m := metrics.Get()

	// max_age is a duration string in server.yml; an unparseable value
	// falls back to the documented 1h default rather than failing startup.
	maxAge, err := time.ParseDuration(mc.Loki.MaxAge)
	if err != nil || maxAge <= 0 {
		maxAge = time.Hour
	}

	wrap := func(token string, h http.Handler) http.HandlerFunc {
		handler := metrics.AuthMiddleware(token, mc.Auth.AllowUnauthenticated, h)
		return handler.ServeHTTP
	}
	prometheusHandler := wrap(mc.Auth.Tokens.Prometheus, m.PrometheusHandler())
	grafanaHandler := wrap(mc.Auth.Tokens.Grafana, m.GrafanaHandler(paths.ProjectName))
	lokiHandler := wrap(mc.Auth.Tokens.Loki, m.LokiHandler(metricsLogSource(), mc.Loki.MaxEntries, maxAge))

	mount := func(prefix string) {
		r.Get(prefix, prometheusHandler)
		r.Get(prefix+"/prometheus", prometheusHandler)
		r.Get(prefix+"/grafana", grafanaHandler)
		r.Get(prefix+"/loki", lokiHandler)
	}
	mount("/server/metrics")
	mount("/api/v1/server/metrics")
	mount("/api/metrics")
	if mc.Root.Enabled {
		mount("/metrics")
	}
...
func manifestHandler(cfg *config.Config) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{
			...
			"short_name":       cfg.Server.Branding.Title,
			...
			// Icon URLs carry the build stamp for the same reason every other
			// static reference does: an installed PWA otherwise keeps serving
			// the icons cached under the previous build.
			"icons": []map[string]string{
				{"src": assetURL("/static/images/icons/icon-192.png"), "sizes": "192x192", "type": "image/png", "purpose": "any"},
				{"src": assetURL("/static/images/icons/icon-512.png"), "sizes": "512x512", "type": "image/png", "purpose": "any"},
				{"src": assetURL("/static/images/icons/icon-192-maskable.png"), "sizes": "192x192", "type": "image/png", "purpose": "maskable"},
				{"src": assetURL("/static/images/icons/icon-512-maskable.png"), "sizes": "512x512", "type": "image/png", "purpose": "maskable"},
			},
		})
	}
}
...
func buildETag() string {
	return fmt.Sprintf(`W/"%s"`, assetStamp())
}

// assetURL appends the running build stamp to a static asset path as the
// `?v=` query value AI.md PART 9 "Asset Version-Busting" mandates, so every
// asset URL changes on a new build and the static handler can serve the
// matching request as immutable.
func assetURL(path string) string {
	if strings.Contains(path, "?") {
		return path + "&v=" + assetStamp()
	}
	return path + "?v=" + assetStamp()
}

// staticAssetHandler serves the embedded static tree under the PART 9 cache
// policy: a request whose `?v=` equals the running build stamp is cached for
// a year as immutable, while a bare or stale-stamped path gets no-cache plus
// the build-stamp ETag. HTML cached by an older build therefore keeps
// working - its asset requests simply revalidate instead of pinning stale
// bytes. The ETag is set before delegating so http.ServeContent answers a
...
func staticAssetHandler() http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Query().Get("v") == assetStamp() {
...
// serviceWorkerVersionToken is the placeholder the embedded sw.js carries in
// place of the build version; serviceWorkerHandler substitutes it at serve
// time so a single embedded asset always advertises the running version.
const serviceWorkerVersionToken = "__SERVER_VERSION__"
...
func serviceWorkerHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// AI.md PART 16 requires the service worker cache name to follow the
		// app's semver (api-cache-v{major}.{minor}.{patch}). The embedded
		// source carries a placeholder so the version is stamped from the
		// running build rather than drifting as a hardcoded literal.
		data = bytes.Replace(data, []byte(serviceWorkerVersionToken), []byte(Version), 1)
...
func apiHashHandler(w http.ResponseWriter, r *http.Request) {
	algorithm := paramValue(r, "algorithm")
	input := paramValue(r, "input")
...
func apiEncodeHandler(w http.ResponseWriter, r *http.Request) {
	encoding := strings.ToLower(paramValue(r, "encoding"))
	input := paramValue(r, "input")
...
func apiCaseHandler(w http.ResponseWriter, r *http.Request) {
	style := strings.ToLower(paramValue(r, "style"))
	input := paramValue(r, "input")
...
func apiBcryptHandler(w http.ResponseWriter, r *http.Request) {
	password := paramValue(r, "password")
	cost := 12
	// Check URL param first, then query param
	if c := paramValue(r, "cost"); c != "" {
...
func apiDateTimeNowHandler(w http.ResponseWriter, r *http.Request) {
	timezone := paramValue(r, "timezone")
	if timezone == "" {
...
func apiConvertTimestampHandler(w http.ResponseWriter, r *http.Request) {
	timestamp, err := strconv.ParseInt(paramValue(r, "timestamp"), 10, 64)
	if err != nil {
...
func apiAddDurationHandler(w http.ResponseWriter, r *http.Request) {
	...
	duration := paramValue(r, "duration")
...
func apiDiffHandler(w http.ResponseWriter, r *http.Request) {
	timestamp1, err := strconv.ParseInt(paramValue(r, "timestamp1"), 10, 64)
	if err != nil {
		errorResponse(w, "invalid timestamp1", http.StatusBadRequest)
		return
	}

	timestamp2, err := strconv.ParseInt(paramValue(r, "timestamp2"), 10, 64)
	if err != nil {

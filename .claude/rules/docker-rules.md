...
- Never require `.env` files or use `${VAR}`/`${VAR:-default}` syntax in compose files — hardcode sane defaults directly, YAML map style (`KEY: value`), never list style (`- KEY=value`).
...
- Never run the runtime process as root unless the app must bind a privileged port (<1024, prefer `setcap` instead) or manage system services — document any exception in IDEA.md.

## CRITICAL - ALWAYS DO
- Always use a multi-stage Dockerfile: builder stage `casjaysdev/go:latest`, runtime stage `alpine:latest`.
- Always create and switch to a non-root user in the runtime stage (`addgroup -S app && adduser -S -G app app` on Alpine; `groupadd -r app && useradd -r -g app app` on Debian/Ubuntu).

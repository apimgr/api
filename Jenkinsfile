pipeline {
    ...
    environment {
        PROJECT_NAME = 'api'
        // Frozen forever, even across a project rename — never change after first-time setup
        INTERNAL_NAME = 'api'
        PROJECT_ORG = 'apimgr'
        BINDIR = 'binaries'
        RELDIR = 'releases'
        ...
        stage('Release: Beta') {
            ...
            steps {
                sh '''
                    ...
                    tar --exclude='.git' --exclude='.github' --exclude='.gitea' --exclude='.forgejo' --exclude='binaries' --exclude='releases' --exclude='*.tar.gz' -czf ${RELDIR}/${PROJECT_NAME}-${VERSION}-source.tar.gz .
                '''
                // SBOM tooling lives in the build image - never install it on the agent
                sh '''
                    docker run --rm \
